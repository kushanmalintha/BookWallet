...
class _UserSearchListviewState extends State<UserSearchListview>
  ...
  final SearchScreenController _searchScreenController =
      SearchScreenController(); // Instance of GoogleusersController
  ...
  void initState() {
    ...
    _searchScreenController.searchUsers(
        _onDataFetched,
        widget
  ...
  void _scrollListener() {
    if (_scrollController.position.pixels ==
      ...
      _searchScreenController.searchUsers(_onDataFetched,
  ...
  Future<void> _onRefresh() async {
    setState(() {
      _searchScreenController.users = [];
      _searchScreenController.currentPage = 1;
    });

    await _searchScreenController.searchUsers(_onDataFetched, widget.searchText);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount: _searchScreenController.users.length +
            1, // Number of items in the list +1 for loading indicator
        itemBuilder: (context, index) {
          if (index < _searchScreenController.users.length) {
            return Column(
              children: [
                const SizedBox(height: 3), // Spacer between cards
                UserCard(
                    user: _searchScreenController.users[index])
              ],
            );
          } else {
